...
def init_llm():
    ...
    log(
        "[LLM] Connexion Ollama: host=%s model=%s timeout=%s temp=%.3f ctx=%s stream=%s"
        % (OLLAMA_HOST, LLM_ID, OLLAMA_TIMEOUT, OLLAMA_TEMPERATURE, OLLAMA_NUM_CTX, bool(OLLAMA_STREAM))
    )
    return None, None
